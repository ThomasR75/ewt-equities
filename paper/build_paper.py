...
DET = {"reads":"27,644","setups":"442","win":"22.6%","wilson":"[19.0, 26.8]%","exp":"-0.01 R",
       "boot":"[-0.17, +0.15]","p0":"0.83","null_mean":"-0.11","null_ci":"[-0.21, -0.01]",
       "p_dir":"0.06","anti":"-0.21","A":"23% win, +0.16 R (n=115)","B":"23% win, -0.07 R (n=327)",
       "A_beats_B":"Yes (+0.16 vs -0.07)","calibrated":"Partial (top-conf +0.17)"}
DRIFT = {"long":"n=249, +0.36 R","short":"n=193, -0.50 R","alllong":"+0.32 R",
         "bh":"+6.05 R","alpha":"-6.06 R","alpha_ci":"[-6.94, -5.26]"}
LLM_DONE = True
LLM = {"model":"qwen2.5:14b-instruct","win":"22.7%","exp":"+0.01 R","boot":"boot95 [-0.14, +0.18], p=0.89",
       "A":"26% win, +0.32 R (n=155)","B":"21% win, -0.14 R (n=311)","A_beats_B":"Yes (+0.32 vs -0.14)","calibrated":"Yes (monotonic)"}
DISCUSSION_LLM = ("On the larger monthly sample the LLM weigher meets two of its four pre-registered criteria: "
  "grade A &gt; B (+0.32R vs &minus;0.14R) and a monotonic confidence&ndash;return relation (top bucket +0.13R, lower "
  "buckets negative), and it ranks setups better than the deterministic grade (A +0.32R vs +0.16R). It fails the two "
  "that would establish an edge: expectancy still spans zero (+0.01R, p=0.89) and even grade-A setups lose to "
  "buy-and-hold (alpha &asymp; &minus;5R) and to an always-long book on the same setups. So the weigher adds real "
  "relative calibration &mdash; it concentrates market drift into its high-grade cells &mdash; without creating net "
  "skill: it sorts, it does not predict. Consistent with &sect;5.2, the bottleneck remains the wave-count features.")
...
def build():
    ...
    e.append(P("We build a deterministic, point-in-time Elliott-Wave (EW) signal generator and an independent reliability tester sharing one set of outcome rules, and ask (1) whether the mechanical engine yields a tradeable, calibrated edge, and (2) whether a local LLM scenario weigher improves it. On an anonymized 50-instrument panel (1962&ndash;2026; 588k daily bars) a monthly-cadence walk-forward produced 442 out-of-sample setups with a 22.6% win rate (Wilson 95% CI 19.0&ndash;26.8%) and &minus;0.01R expectancy (bootstrap 95% CI &minus;0.17 to +0.15; p&asymp;0.83): a precise zero. (A smaller annual run showed +0.35R; the 6.5&times;-larger monthly sample dissolves that to zero, i.e. it was drift/noise.) The result is market drift, not wave skill: long setups profit (+0.36R) while shorts lose (&minus;0.50R), an all-long book matches the engine, and against buy-and-hold the engine's alpha is significantly negative (&minus;6.06R, 95% CI &minus;6.94 to &minus;5.26). A calibration signal does emerge with the larger sample &mdash; grade A beats grade B and the top confidence bucket is positive &mdash; but even grade-A setups lose to buy-and-hold, so the weigher ranks (concentrates drift) without creating an edge. Two further controls agree: tuning the engine's most powerful parameter created an edge that vanished once both the parameter and the instruments were held out (p=0.59), and a gradient-boosted weigher's apparent skill was entirely the direction sign (drift), with wave-structure features at chance (AUC 0.53). The local-LLM weigher did not help &mdash; expectancy edged down and grade calibration worsened. We report a reproducible null: no detectable wave-counting edge, and a mechanization that underperforms buy-and-hold.",s,"Abs"))
    ...
    e.append(P(DET["reads"]+" point-in-time signal records across the 50 instruments (monthly-cadence walk-forward, full history) yielded "+DET["setups"]+" distinct setups. A smaller annual run gave 68 setups at +0.35R; that positive dissolved to zero here under 6.5&times; the sample, and is reported as the drift artifact it was.",s))
    ...
    e.append(Spacer(1,5))
    e.append(P("<b>Reading.</b> Expectancy is a precise zero and the engine loses to buy-and-hold by &minus;6.06R: profit comes only from long setups, the short calls lose, and an all-long book matches the engine, so its directional choices add nothing over drift. Unlike the small annual run, the larger sample does reveal a modest calibration signal &mdash; grade A (+0.16R) beats grade B (&minus;0.07R) and the highest-confidence bucket is positive (+0.17R) &mdash; i.e. the engine can rank setups relatively. But the ranking sorts drift, not skill: even grade-A setups lose to buy-and-hold (alpha &asymp; &minus;6R) and an always-long book on the same grade-A setups (+0.45R) beats grade A. Net: no wave-counting edge; the weighting layer can order setups but only by concentrating drift &mdash; examined next.",s))
    ...
    e.append(P("A reproducible, pre-registered harness gives a precise answer on this universe: the mechanical engine has no statistically detectable out-of-sample edge and no wave-counting skill. Expectancy is a precise zero on a 442-setup monthly sample (a smaller annual run's +0.35R dissolved under 6.5&times; the data), and the engine underperforms buy-and-hold by &minus;6.06R. A calibration signal does emerge with power &mdash; grade and confidence order setups, and the local-LLM weigher ranks them better, meeting two of its four pre-registered criteria &mdash; but even the best-graded setups lose to buy-and-hold, so the weigher concentrates drift without creating an edge. Neither tuning the most powerful parameter (an apparent edge that vanished once parameter and instruments were both held out) nor a learned weigher (whose skill was drift, not structure) nor the local-LLM weigher changed the bottom line. The bottleneck is the information in the wave-count features, not the weighting model &mdash; reproducing, inside our own pipeline, the data-snooping and no-edge cautions of the technical-analysis literature [3,4].",s))
